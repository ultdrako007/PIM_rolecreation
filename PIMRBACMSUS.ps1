...
.DESCRIPTION
    ...
    - Extraccion de roles integrada (-ExtractRoles)
    - Lista blanca por RoleDefinitionId (GUID) en lugar de nombres
    - Deteccion de roles heredados por ancestros de scope
    - Migracion automatica controlada (crear elegible + revocar permanente)
    - CORRECCIONES: Regex, funciones duplicadas, paginacion, rate limiting
...
.PARAMETER AutoMigrate
    Habilita migracion automatica de roles permanentes a PIM
    
.PARAMETER ExtractRoles
    Modo extraccion de roles (no requiere CsvPath ni Phase)
    
.PARAMETER DryRun
    Modo simulacion
    
.PARAMETER AllowedRolesConfigPath
    Ruta al archivo JSON de configuracion de roles permitidos
...
param(
    ...
    # Parametros para extraccion de roles
    [switch]$ExtractRoles,
    [switch]$ExportRolesCsv,
    [string]$RolesOutputPath = "roles_reference.csv",
    
    # Soporte multi-suscripcion
    [switch]$AllSubscriptions,
    [string[]]$SubscriptionIds,
    
    # CORRECCIiìN: Agregar parametro faltante
...
function Import-AllowedRolesConfiguration {
    <#
    .SYNOPSIS
        Carga configuracion de roles permitidos desde archivo externo JSON
    #>
    [CmdletBinding()]
    param([string]$ConfigPath)
    
    Write-Log "Cargando configuracion de roles desde: $ConfigPath" -Level Info
    
    if (-not (Test-Path $ConfigPath)) {
        Write-Log "Archivo de configuracion no encontrado, usando configuracion por defecto embebida" -Level Warning
        return $false
    }
    ...
    try {
        ...
        # Calcular hash del archivo de configuracion
        $script:AllowedRolesConfigHash = (Get-FileHash -Path $ConfigPath -Algorithm SHA256).Hash
        Write-Log "Hash de configuracion: $($script:AllowedRolesConfigHash.Substring(0,16))..." -Level Info
        ...
        Write-Log "Error cargando configuracion: $_" -Level Error
        Write-Log "Usando configuracion por defecto embebida" -Level Warning
        return $false
    }
}
        ...
        try {
            ...
            if ($_.Exception.Message -match '429|TooManyRequests|throttl') {
                ...
                if ($attempt -ge $MaxRetries) {
                    throw "Fallo despues de $MaxRetries intentos por throttling"
                }
...
function Test-TokenExpiration {
    <#
    .SYNOPSIS
        NUEVO: Valida expiracion de token y reconecta si es necesario
    #>
    [CmdletBinding()]
    param()
    ...
    try {
        ...
        Write-Verbose "No se pudo verificar expiracion de token: $_"
    }
}
...
function Get-AllRoleDefinitions {
    ...
    Write-Host "‚ïë  Extractor de RoleDefinitionId (GUIDs)        ‚ïë" -ForegroundColor Cyan
    ...
    try {
        ...
        # Roles cri≠ticos
        Write-Host "`n--- ROLES CRiçTICOS DE ENTRA ID ---" -ForegroundColor Yellow
        $criticalEntraIds = @(
    ...
    if ($SupportAzureRBAC) {
        ...
        try {
            ...
            if ($azContext) {
                Write-Log "Suscripcion activa: $($azContext.Subscription.Name)" -Level Info
                ...
                # Roles cri≠ticos de Azure
                Write-Host "`n--- ROLES CRiçTICOS DE AZURE RBAC ---" -ForegroundColor Yellow
    ...
    if ($ExportCsv) {
        try {
            ...
            Write-Log "`n‚úì CSV exportado: $OutputPath" -Level Success
        }
        catch {
    ...
    Write-Host "`n‚úì Extraccion completada" -ForegroundColor Green
    return $allRoles
}
...
function Get-ScopeAncestors {
    <#
    .SYNOPSIS
        CORREGIDO: Obtiene ancestros de un scope (recurso ‚Üí RG ‚Üí suscripcion)
    #>
    [CmdletBinding()]
    param([string]$Scope)
    ...
    # TODO: Si en el futuro se necesita Management Groups, agregar aqui≠
    # Get-AzManagementGroup -GroupName "..." para resolver jerarqui≠a
...
function Get-UserGroupMemberships {
    ...
    try {
        ...
        Write-Verbose "Error obteniendo membresi≠as de grupo: $_"
        return @()
    }
}
...
function Test-ExistingAzureRoleAssignmentWithAncestors {
    <#
    .SYNOPSIS
        CORREGIDO: Detecta asignaciones permanentes con paginacion y deteccion de grupos
    #>
    [CmdletBinding()]
    param(
        [string]$PrincipalId,
    ...
    # MEJORA: Obtener grupos del usuario para deteccion precisa
    $userGroups = Get-UserGroupMemberships -PrincipalId $PrincipalId
    
    foreach ($ancestorScope in $ancestors) {
        try {
            # CORRECCIiìN: Agregar paginacion y reintentos
            ...
            if ($assignments) {
                foreach ($assignment in $assignments) {
                    ...
                    if ($isGroupInherited) {
                        Write-Log "‚ö†Ô∏è  ASIGNACIiìN HEREDADA POR GRUPO detectada:" -Level Warning
                        ...
                        Write-Log "    ‚Üí Accion requerida: Migrar grupo o remover usuario del grupo" -Level Warning
                    }
    ...
    if ($conflictingAssignments.Count -gt 0) {
        ...
        Write-Log "CONFLICTO AZURE: $($conflictingAssignments.Count) asignacion(es) de '$roleName'" -Level Warning
        Write-Log "  - Directas: $directAssignments (removibles con -AutoMigrate)" -Level Warning
        Write-Log "  - Por Grupo: $groupAssignments (requieren accion manual)" -Level Warning
        
        foreach ($conflict in $conflictingAssignments) {
            $inheritedMsg = if ($conflict.IsInherited) { " (HEREDADO)" } else { "" }
            $groupMsg = if ($conflict.IsGroupInherited) { " [ViçA GRUPO: $($conflict.GroupDisplayName)]" } else { "" }
            Write-Log "  ‚Üí Scope: $($conflict.Scope)$inheritedMsg$groupMsg" -Level Warning
        }
...
function Remove-AzureRoleAssignmentSafe {
    <#
    .SYNOPSIS
        Remueve asignacion permanente Azure - SOLO si no es heredada por grupo
    #>
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        if ($Assignment.IsGroupInherited) {
            Write-Log "‚ö†Ô∏è  SKIP: Asignacion heredada por GRUPO no se puede remover automaticamente" -Level Warning
            Write-Log "    Grupo: $($Assignment.GroupDisplayName)" -Level Warning
            Write-Log "    Accion requerida: Remover usuario del grupo o migrar grupo a PIM" -Level Warning
        ...
        if ($DryRun) {
            Write-Log "[DRY-RUN] Removeri≠a: $roleName en $($Assignment.Scope)" -Level Warning
            return $true
        }
        
        Write-Log "Removiendo permanente: $roleName en $($Assignment.Scope)..." -Level Warning
        
        # Usar retry en remocion
        ...
        Write-Log "‚úì Asignacion permanente removida" -Level Success
        ...
        Write-Log "Error removiendo asignacion: $_" -Level Error
...
function Test-ExistingAzureEligibility {
    <#
    .SYNOPSIS
        CORREGIDO: Verifica eligibilidad existente con paginacion y reintentos
    #>
    [CmdletBinding()]
    param(
        [string]$PrincipalId,
    ...
    try {
        # CORRECCIiìN: Agregar reintentos
...
function Test-ExistingEntraRoleAssignment {
    ...
    try {
        ...
        if ($existing) {
            Write-Log "CONFLICTO ENTRA ID: Asignacion permanente existe" -Level Warning
            
            $script:Statistics.Conflicts++
            Add-AuditEntry -PrincipalId $PrincipalId -RoleOrGroup $RoleDefinitionId `
                -Action "EntraConflictDetected" -Status "Warning" `
                -Details "Asignacion permanente: $($existing[0].Id)"
...
function Remove-EntraRoleAssignmentSafe {
    <#
    .SYNOPSIS
        Remueve asignacion permanente de Entra ID de forma segura
    #>
    [CmdletBinding()]
    param(
        [PSCustomObject]$Assignment,
    ...
    try {
        if ($DryRun) {
            Write-Log "[DRY-RUN] Removeri≠a asignacion Entra ID permanente: $($Assignment.RoleDefinitionId)" -Level Warning
            return $true
        }
        
        Write-Log "Removiendo asignacion permanente Entra ID..." -Level Warning
        
        Remove-MgRoleManagementDirectoryRoleAssignment -UnifiedRoleAssignmentId $Assignment.Id -ErrorAction Stop
        
        Write-Log "‚úì Asignacion permanente Entra ID removida" -Level Success
        ...
        Write-Log "Error removiendo asignacion Entra ID: $_" -Level Error
        return $false
    }
}
...
function Test-EntraDurationCompliance {
    <#
    .SYNOPSIS
        Verifica li≠mites de duracion para roles Entra ID
    #>
    [CmdletBinding()]
    param(
        [string]$RoleDefinitionId,
    ...
    if ($requestedDuration -gt $maxDuration) {
        Write-Log "Duracion excede li≠mite Entra ID: $Duration > $($roleConfig.MaxDuration)" -Level Warning
        return $false
    }
...
function Test-ExistingGroupEligibility {
    <#
    .SYNOPSIS
        CORREGIDO: Verifica eligibilidad existente en grupo PIM con paginacion
    #>
    [CmdletBinding()]
    param(
        [string]$PrincipalId,
    ...
    try {
        ...
        # CORRECCIiìN: Agregar reintentos y paginacion
        $existing = Invoke-WithRetry -ScriptBlock {
        ...
        if ($existing) {
            Write-Log "IDEMPOTENCIA GRUPO: Membresi≠a ya existe" -Level Warning
            $script:Statistics.Skipped++
            Add-AuditEntry -PrincipalId $PrincipalId -RoleOrGroup $GroupId `
                -Action "GroupSkipped" -Status "AlreadyExists" `
                -Details "Membresi≠a existente en grupo"
            return $true
        }
    }
    catch {
...
function New-GroupEligibility {
    <#
    .SYNOPSIS
        Crea eligibilidad de membresi≠a en grupo PIM
    #>
    [CmdletBinding()]
    param(
        [string]$PrincipalId,
    ...
    try {
        ...
        throw "Error creando membresi≠a grupo: $_"
    }
}
...
function Test-Prerequisites {
    ...
    foreach ($module in $requiredModules) {
        if (-not (Get-Module -ListAvailable -Name $module)) {
            throw "ERROR: Modulo requerido no instalado: $module"
        }
        Import-Module $module -ErrorAction Stop
        Write-Log "Modulo cargado: $module" -Level Success
    }
    ...
    # NUEVO: Validar expiracion de token
...
function Test-AzureRBACPrerequisites {
    ...
    Write-Log "Suscripcion actual: $($azContext.Subscription.Name)" -Level Info
    ...
    # NUEVO: Validar acceso a cada suscripcion
    $accessibleSubs = @()
    foreach ($sub in $subscriptionsToProcess) {
        try {
            Set-AzContext -SubscriptionId $sub.Id -ErrorAction Stop | Out-Null
            Write-Log "  ‚úì $($sub.Name) ($($sub.Id))" -Level Success
            $accessibleSubs += $sub
        }
        catch {
            Write-Log "  ‚úó Sin acceso: $($sub.Name)" -Level Warning
        }
    }
    
    if ($accessibleSubs.Count -eq 0) {
        throw "ERROR: Sin acceso a ninguna suscripcion"
    }
    
    # Restaurar contexto original
    Set-AzContext -SubscriptionId $azContext.Subscription.Id | Out-Null
    
    # Inicializar cache con roles de la suscripcion actual
...
function Get-SubscriptionsToProcess {
    <#
    .SYNOPSIS
        Determina que suscripciones procesar segun parametros
    #>
    [CmdletBinding()]
    param()
    ...
    elseif ($SubscriptionIds) {
        Write-Log "Modo: Suscripciones especi≠ficas ($($SubscriptionIds.Count))" -Level Info
        $subs = @()
        foreach ($subId in $SubscriptionIds) {
            ...
            if ($sub) {
                ...
                Write-Log "Advertencia: Suscripcion no encontrada: $subId" -Level Warning
            }
        }
        return $subs
    }
        ...
        Write-Log "Modo: Solo suscripcion actual" -Level Info
...
function Set-AzureSubscriptionContext {
    <#
    .SYNOPSIS
        Cambia el contexto a una suscripcion especi≠fica
    #>
    [CmdletBinding()]
    ...
    try {
        ...
        Write-Verbose "Contexto cambiado a suscripcion: $($context.Subscription.Name)"
        
        # Actualizar cache de roles para esta suscripcion
        $script:AzureRoleDefinitionsCache = @{}
        Initialize-AzureRoleDefinitionsCache
        ...
        Write-Log "Error cambiando a suscripcion $SubscriptionId: $_" -Level Error
        throw
    }
}
...
function Test-CsvStructureExtended {
    ...
    Write-Log "Estructura CSV valida: $($CsvData.Count) filas" -Level Success
}

function Test-InputValidations {
    <#
    .SYNOPSIS
        Valida formatos de entrada en cada fila del CSV
    .DESCRIPTION
        Verifica GUIDs, formato ISO-8601, y longitud de justificacion
    #>
    [CmdletBinding()]
    param([hashtable]$Row)
    ...
    try { 
        ...
        $errors += "PrincipalId invalido: $($Row.PrincipalId)" 
    }
    ...
    if ($Row.RoleDefinitionId) {
        try { 
            ...
            $errors += "RoleDefinitionId invalido: $($Row.RoleDefinitionId)" 
        }
    }
    
    if ($Row.AzureRoleDefinitionId) {
        try { 
            [System.Guid]::Parse($Row.AzureRoleDefinitionId) | Out-Null 
        }
        catch { 
            $errors += "AzureRoleDefinitionId invalido: $($Row.AzureRoleDefinitionId)" 
        }
    }
    ...
    if ($Row.GroupId) {
        try { 
            ...
            $errors += "GroupId invalido: $($Row.GroupId)" 
        }
    }
    
    # CORRECCION APLICADA: Regex ISO-8601 completa con $ final
    $durationPattern = '^P(?:\d+Y)?(?:\d+M)?(?:\d+W)?(?:\d+D)?(?:T(?:\d+H)?(?:\d+M)?(?:\d+(?:\.\d+)?S)?)?$'
    
    if ($Row.Duration -notmatch $durationPattern) {
        $errors += "Duration formato ISO-8601 invalido: $($Row.Duration)"
    }
    
    if ($Row.Reason.Length -lt 5) {
        $errors += "Justificacion insuficiente (minimo 5 caracteres)"
    }
...
function New-PIMEligibilityAssignmentExtended {
    ...
    try {
        ...
        if ($isAzureRole) {
            ...
            if ($conflictCheck.HasConflict) {
                if ($conflictCheck.HasGroupInherited) {
                    Write-Log "‚ö†Ô∏è  Asignaciones heredadas por GRUPO detectadas" -Level Warning
                    Write-Log "    AutoMigrate solo procesara asignaciones directas" -Level Warning
                }
                ...
                if ($AutoMigrate) {
                    ...
                    if (-not $DryRun) {
                        ...
                        Write-Log "‚úì Eligibilidad Azure creada" -Level Success
                        ...
                        if ($groupInheritedAssignments.Count -gt 0) {
                            $details += ", $($groupInheritedAssignments.Count) heredadas por grupo requieren accion manual"
                        }
            ...
            if ($Phase -ge 1) {
                ...
                if ($roleConfig) {
                    ...
                    if ($requestedDuration -gt $maxDuration) {
                        $roleName = Get-AzureRoleNameById -RoleDefinitionId $Row.AzureRoleDefinitionId
                        Write-Log "Duracion excede li≠mite: $($Row.Duration) > $($roleConfig.MaxDuration)" -Level Warning
                        $script:Statistics.Failed++
                        return
                    }
                }
            ...
            $roleName = Get-AzureRoleNameById -RoleDefinitionId $Row.AzureRoleDefinitionId
            Write-Log "‚úì Eligibilidad Azure creada: $roleName" -Level Success
            $script:Statistics.Success++
            $script:Statistics.AzureRoles++
            Add-AuditEntry -PrincipalId $Row.PrincipalId -RoleOrGroup $roleName `
        ...
        if ($isEntraRole) {
            ...
            if ($conflictCheck.HasConflict) {
                if ($AutoMigrate) {
                    ...
                    if (-not $DryRun) {
                        ...
                        Write-Log "‚úì Eligibilidad Entra ID creada" -Level Success
                        ...
                        Write-Log "[DRY-RUN] Se creari≠a eligibilidad Entra ID y removeri≠an $($conflictCheck.Assignments.Count) permanentes" -Level Warning
                        $script:Statistics.Success++
                        $script:Statistics.EntraRoles++
                    }
                }
            ...
            Write-Log "‚úì Eligibilidad Entra ID creada: $($Row.RoleDefinitionId)" -Level Success
            $script:Statistics.Success++
            $script:Statistics.EntraRoles++
            Add-AuditEntry -PrincipalId $Row.PrincipalId -RoleOrGroup $Row.RoleDefinitionId `
        ...
        if ($isGroup) {
            ...
            if ($DryRun) {
                Write-Log "[DRY-RUN] Membresi≠a grupo: $($Row.GroupId)" -Level Info
                $script:Statistics.Success++
            ...
            Write-Log "‚úì Membresi≠a grupo creada" -Level Success
            $script:Statistics.Success++
            $script:Statistics.Groups++
        ...
        Write-Log "Fila omitida: sin tipo valido" -Level Warning
        $script:Statistics.Skipped++
    }
    catch {
...
function Start-PIMAssignmentExtended {
    ...
    if ($AutoMigrate) {
        Write-Host "‚ö†Ô∏è  AUTO-MIGRATE ACTIVO: Roles permanentes DIRECTOS seran migrados" -ForegroundColor Yellow
        Write-Host "‚ö†Ô∏è  Roles heredados por GRUPO requeriran accion manual" -ForegroundColor Yellow
    }
    
    if ($AllSubscriptions) {
        Write-Host "üåê MULTI-SUSCRIPCIiìN: Procesando todas las suscripciones del tenant" -ForegroundColor Cyan
    }
    elseif ($SubscriptionIds) {
        Write-Host "üéØ MULTI-SUSCRIPCIiìN: Procesando $($SubscriptionIds.Count) suscripciones especi≠ficas" -ForegroundColor Cyan
    }
    ...
    if (Test-Path $AllowedRolesConfigPath) {
        ...
        if ($configLoaded) {
            Write-Host "‚úì Configuracion cargada desde: $AllowedRolesConfigPath" -ForegroundColor Green
        }
    }
    ...
    # Agrupar asignaciones por suscripcion
    ...
    foreach ($row in $csvData) {
        if ($row.AzureScope) {
            if ($row.AzureScope -match '/subscriptions/([^/]+)') {
                ...
                Write-Log "Advertencia: Scope invalido en fila: $($row.AzureScope)" -Level Warning
            }
        }
        else {
    ...
    if (-not $Force -and -not $DryRun) {
        Write-Host "`n‚ö†Ô∏è  CONFIRMACIiìN:" -ForegroundColor Yellow
        ...
        Write-Host "  Modo: PRODUCCIiìN" -ForegroundColor White
        if ($AutoMigrate) {
            Write-Host "  AutoMigrate: HABILITADO" -ForegroundColor Yellow
        }
        
        $confirm = Read-Host "`n¬øContinuar? (escriba 'SI')"
        if ($confirm -ne 'SI') {
            Write-Log "Operacion cancelada" -Level Warning
            return
        }
    }
    ...
    # Procesar asignaciones Azure RBAC por suscripcion
    if ($SupportAzureRBAC -and $assignmentsBySubscription.Keys.Count -gt 0) {
        ...
        foreach ($subId in $assignmentsBySubscription.Keys) {
            ...
            if (-not $shouldProcess) {
                Write-Log "Omitiendo suscripcion $subId (no en scope de procesamiento)" -Level Warning
            ...
            if (-not $subscription) {
                Write-Log "Suscripcion no encontrada: $subId" -Level Error
                $script:Statistics.Failed += $assignments.Count
                continue
            }
            
            Write-Log "`n=== Procesando Suscripcion [$currentSubIndex/$($assignmentsBySubscription.Keys.Count)]: $($subscription.Name) ($subId) ===" -Level Info
            Write-Log "Asignaciones en esta suscripcion: $($assignments.Count)" -Level Info
            ...
            try {
                ...
                Write-Log "Error cambiando a suscripcion $($subscription.Name): $_" -Level Error
                $script:Statistics.Failed += $assignments.Count
                continue
            }
            ...
            foreach ($row in $assignments) {
                $progress++
                Write-Progress -Activity "Procesando suscripcion: $($subscription.Name)" `
            ...
            Write-Progress -Activity "Procesando suscripcion: $($subscription.Name)" -Completed
        }
    }
    ...
    if ($DryRun) {
        Write-Log "‚úì Simulacion completada" -Level Success
    }
    else {
        Write-Log "‚úì Procesamiento completado" -Level Success
    }
}
...
function Export-AuditReportExtended {
    ...
    Write-Host "`n‚ïî‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïó" -ForegroundColor Cyan
    Write-Host "‚ïë  RESUMEN DE EJECUCIiìN              ‚ïë" -ForegroundColor Cyan
    ...
    if ($AutoMigrate) {
        ...
        if ($script:Statistics.GroupInheritedSkipped -gt 0) {
            Write-Host "  ‚ö†Ô∏è  Heredados por grupo (requieren accion manual): $($script:Statistics.GroupInheritedSkipped)" -ForegroundColor Yellow
        }
    }
...
if ($ExtractRoles) {
    try {
        ...
        Write-Log "Error en extraccion: $_" -Level Error
    }
    exit 0
}
